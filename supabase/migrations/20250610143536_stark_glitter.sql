/*
  ...
  2. Add tables to realtime publication (with checks to avoid errors)
...
-- Add photos table to publication if not already a member
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 
...
-- Add collages table to publication if not already a member
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 
...
-- Add collage_settings table to publication if not already a member
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 
