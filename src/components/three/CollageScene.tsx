...
const CollageScene: React.FC<CollageSceneProps> = ({ photos, settings, onSettingsChange }) => {
  // Get photos directly from the store for real-time updates
  const storePhotos = useCollageStore(state => state.photos);
  const lastRefreshTime = useCollageStore(state => state.lastRefreshTime);
  
  // Use either passed photos or store photos, preferring store photos for real-time updates
  // CRITICAL FIX: Always use store photos for real-time updates
  const effectivePhotos = storePhotos;
  
  // Track when photos change for debugging
  useEffect(() => {
    console.log('🖼️ SCENE: Photos updated, count:', effectivePhotos.length);
    console.log('🖼️ SCENE: Photo IDs:', effectivePhotos.map(p => p.id.slice(-4)).join(', '));
  }, [effectivePhotos]);
  
  // Track refresh time for debugging
  useEffect(() => {
    if (lastRefreshTime > 0) {
      console.log('🖼️ SCENE: Store refreshed at:', new Date(lastRefreshTime).toLocaleTimeString());
    }
  }, [lastRefreshTime]);
  ...
  const safePhotos = Array.isArray(effectivePhotos) ? effectivePhotos.filter(p => p && p.id) : [];
  ...
  return (
    <div style={backgroundStyle} className="w-full h-full">
      ...
      >
        ...
        <PhotoDebugger photos={effectivePhotos} />
        
        {photosWithPositions.map((photo) => (
          <PhotoMesh
            key={`${photo.id}-${photo.slotIndex}-${effectivePhotos.length}-v3`}
            photo={photo}
